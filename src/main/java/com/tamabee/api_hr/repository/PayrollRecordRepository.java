...
public interface PayrollRecordRepository extends JpaRepository<PayrollRecordEntity, Long> {
    ...
    /**
     * Tìm bản ghi lương theo ID (chưa bị xóa)
     */
    Optional<PayrollRecordEntity> findByIdAndDeletedFalse(Long id);

    /**
     * Lấy danh sách bản ghi lương của công ty theo kỳ (phân trang)
     */
    @Query("SELECT p FROM PayrollRecordEntity p WHERE p.deleted = false " +
     ...
     */
    Optional<PayrollRecordEntity> findByEmployeeIdAndYearAndMonthAndDeletedFalse(
     ...
     */
    @Query("SELECT COUNT(p) FROM PayrollRecordEntity p WHERE p.deleted = false " +
            "AND p.companyId = :companyId " +
            "AND p.year = :year AND p.month = :month")
     ...
     */
    boolean existsByEmployeeIdAndYearAndMonthAndDeletedFalse(
            Long employeeId, Integer year, Integer month);
